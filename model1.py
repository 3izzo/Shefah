...
class ShefahModel(object):
    def __init__(self, img_c=1, img_w=frame_w, img_h=frame_h, frames_n=max_frame_count, absolute_max_string_len=max_label_length, output_size=max_letter_index):
        self.img_c = img_c
        self.img_w = img_w
        self.img_h = img_h
        ...
        self.input_data = Input(
            name='the_input', shape=input_shape, dtype='float32')

        self.zero1 = ZeroPadding3D(
            padding=(1, 2, 2), name='zero1')(self.input_data)
        self.conv1 = Conv3D(32, (3, 5, 5), strides=(
            1, 2, 2), kernel_initializer='he_normal', name='conv1')(self.zero1)
        ...
        self.maxp1 = MaxPooling3D(pool_size=(1, 2, 2), strides=(
            1, 2, 2), name='max1')(self.drop1)

        self.zero2 = ZeroPadding3D(padding=(1, 2, 2), name='zero2')(self.maxp1)
        self.conv2 = Conv3D(64, (3, 5, 5), strides=(
            1, 1, 1), kernel_initializer='he_normal', name='conv2')(self.zero2)
        ...
        self.maxp2 = MaxPooling3D(pool_size=(1, 2, 2), strides=(
            1, 2, 2), name='max2')(self.drop2)

        self.zero3 = ZeroPadding3D(padding=(1, 1, 1), name='zero3')(self.maxp2)
        self.conv3 = Conv3D(96, (3, 3, 3), strides=(
            1, 1, 1), kernel_initializer='he_normal', name='conv3')(self.zero3)
        ...
        self.maxp3 = MaxPooling3D(pool_size=(1, 2, 2), strides=(
            1, 2, 2), name='max3')(self.drop3)
        ...
        self.gru_2 = Bidirectional(GRU(
            256, return_sequences=True, kernel_initializer='Orthogonal', name='gru2'), merge_mode='concat')(self.gru_1)

        # transforms RNN output to character activations:
        self.dense1 = Dense(
            self.output_size, kernel_initializer='he_normal', name='dense1')(self.gru_2)

        self.y_pred = Activation('softmax', name='softmax')(self.dense1)

        self.labels = Input(name='the_labels', shape=[
                            self.absolute_max_string_len], dtype='float32')
        ...
        self.model = Model(inputs=[self.input_data, self.labels,
...
if(latest):
    ...
    inputs = {'the_input': x_train,
              'the_labels': y_train,
              'input_length': np.array([max_frame_count] * len(x_train)),
              'label_length': np.array([max_label_length] * len(x_train)),
              }
    outputs = {'ctc': np.zeros([len(x_train)])}
    model.fit(inputs, outputs, epochs=300, callbacks=[cp_callback])
...
def decode_predict_ctc(out, top_paths=5):
    ...
    for i in range(top_paths):
        res = K.get_value(K.ctc_decode(out, input_length=np.ones(out.shape[0])*out.shape[1],
...
def test_model(x, y):
    ...
    for i in range(len(y)):
        ...
        total_wer += wordError
        print("Predicted: ", predicted_as_numbers, ", Actual:",
              actual_as_numbers, predicted, actual, wordError)
